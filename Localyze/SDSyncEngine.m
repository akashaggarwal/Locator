...
//    NSString *parseClassName = [entityName componentsSeparatedByString:@";"][1];
//    NSString *localClassName = [entityName componentsSeparatedByString:@";"][0];
...
- (void)downloadDataForRegisteredObjects:(BOOL)useUpdatedAtDate toDeleteLocalRecords:(BOOL)toDelete {
    ...
    for (NSString *className in self.registeredClassesToSync) {
        ...
        NSLog(@"Inside downloadDataForRegisteredObjects parse class name is %@ and local classname is %@", parseClassName, localClassName);
        ...
        AFHTTPRequestOperation *operation = [[SDAFParseAPIClient sharedSDAFParseAPIClient]
                                             ...
                                             success:^(AFHTTPRequestOperation *operation, id responseObject) {
                                                 ...
                                                 //
                                                 NSLog(@"Request for class %@ failed with error: %@", parseClassName, error);
                                                 dispatch_group_leave(group);
                                             }];
...
- (void)postLocalObjectsToServer {
    ...
    for (NSString *className in self.registeredClassesToSync) {
        ...
        NSString *parseClassName = [className componentsSeparatedByString:@";"][1];
        NSString *localClassName = [className componentsSeparatedByString:@";"][0];
        NSLog(@"Inside postLocalObjectsToServer parse class name is %@ and local classname is %@", parseClassName, localClassName);
        //
        // Fetch all objects from Core Data whose syncStatus is equal to SDObjectCreated
        //
        NSArray *objectsToCreate = [self managedObjectsForClass:localClassName withSyncStatus:SDObjectCreated];
        //
        ...
        for (NSManagedObject *objectToCreate in objectsToCreate) {
            ...
            AFHTTPRequestOperation *operation = [[SDAFParseAPIClient sharedSDAFParseAPIClient] POSTRequestForClass:parseClassName parameters:jsonString success:^(AFHTTPRequestOperation *operation, id responseObject) {
                //
...
- (void)deleteObjectsOnServer {
    ...
    for (NSString *className in self.registeredClassesToSync) {
        ...
        NSString *parseClassName = [className componentsSeparatedByString:@";"][1];
        NSString *localClassName = [className componentsSeparatedByString:@";"][0];
        NSLog(@"Inside deleteObjectsOnServer parse class name is %@ and local classname is %@", parseClassName, localClassName);
        
        
        NSArray *objectsToDelete = [self managedObjectsForClass:localClassName withSyncStatus:SDObjectDeleted];
        //
        ...
        for (NSManagedObject *objectToDelete in objectsToDelete) {
            ...
            //
            AFHTTPRequestOperation *operation = [[SDAFParseAPIClient sharedSDAFParseAPIClient] DELETERequestForClass:parseClassName forObjectWithId:[objectToDelete valueForKey:@"objectId"] success:^(AFHTTPRequestOperation *operation, id responseObject) {
...
- (void)writeJSONResponse:(id)response toDiskForClassWithName:(NSString *)className {
    ...
    NSLog(@"inside writeJSONResponse %@ and fileurl is %@", className, fileURL);

    //NSLog(@"inside writeJSONResponse %@", response);
...
- (void)processJSONDataRecordsIntoCoreData {
    ...
    for (NSString *className in self.registeredClassesToSync) {
        ...
        NSString *parseClassName = [className componentsSeparatedByString:@";"][1];
        NSString *localClassName = [className componentsSeparatedByString:@";"][0];
        NSLog(@"Inside processJSONDataRecordsIntoCoreData parse class name is %@ and local classname is %@", parseClassName, localClassName);
        ...
        [managedObjectContext performBlockAndWait:^{
            ...
            if (![managedObjectContext save:&error]) {
                NSLog(@"Unable to save context for class %@", localClassName);
            }
        }];
...
- (void)processJSONDataRecordsForDeletion {
    ...
    for (NSString *className in self.registeredClassesToSync) {
        ...
        NSString *parseClassName = [className componentsSeparatedByString:@";"][1];
        NSString *localClassName = [className componentsSeparatedByString:@";"][0];
        NSLog(@"Inside processJSONDataRecordsForDeletion parse class name is %@ and local classname is %@", parseClassName, localClassName);
        
        NSArray *JSONRecords = [self JSONDataRecordsForClass:parseClassName sortedByKey:@"objectId"];
        if ([JSONRecords count] > 0) {
            ...
            NSArray *storedRecords = [self
                                      managedObjectsForClass:localClassName
